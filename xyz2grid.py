...
def xyzi2grid_v2(x_var, y_var, z_var, i_var, X_RES=512, Y_RES=512, H_RES=120):
    ...
    tmp = grids[0, :, :, :].view(-1)
    index_1 = i_smaller * X_RES * 120 + j_smaller * 120 + k_smaller
    index_2 = i_smaller * X_RES * 120 + j_smaller * 120 +  k_bigger
    index_3 = i_smaller * X_RES * 120 + j_bigger * 120 + k_smaller
    index_4 = i_smaller * X_RES * 120 + j_bigger * 120 + k_bigger
    index_5 = i_bigger * X_RES * 120 + j_smaller * 120 + k_smaller
    index_6 = i_bigger * X_RES * 120 + j_smaller * 120 + k_bigger
    index_7 = i_bigger * X_RES * 120 + j_bigger * 120 + k_smaller
    index_8 = i_bigger * X_RES * 120 + j_bigger * 120 + k_bigger
    ...
    grids[0, :, :, :] = tmp.view(X_RES, Y_RES, 120)
    del tmp
    ...
    tmp = tmp.index_add_(0, index_7, weight110)
    tmp = tmp.index_add_(0, index_8, weight111)
    grids[1, :, :, :] = tmp.view(X_RES, Y_RES, 120)
    del tmp
    return grids
...
def gridi2feature_v2(grids, direction, dist, X_RES=512, Y_RES=512):
    MIN_H = -5
    MAX_H = 5
    H_RES = 120
    ...
    dire = direction.reshape([X_RES,Y_RES,1])
    distt = dist.reshape([X_RES,Y_RES,1])
...
def grid2feature_v2(grids):
    # from apollo 5.5, use_constant_features = False (c++), which means no use of distance and direction
    MIN_H = -5
    MAX_H = 5
    H_RES = 120
    input_cnt_3d = grids
