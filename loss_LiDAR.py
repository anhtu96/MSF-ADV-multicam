...
def lossRenderAttack(outputPytorch, vertex, vertex_og, face, mu):
    ...
    inv_res_x = 0.5 * float(outputPytorch[1].shape[2]) / 60

    x_var = vertex[:, 0]
    y_var = vertex[:, 1]


    fx = torch.floor(x_var * outputPytorch[1].shape[2] / 120 + outputPytorch[1].shape[2] / 2).long()
    fy = torch.floor(y_var * outputPytorch[1].shape[2] / 120 + outputPytorch[1].shape[2] / 2).long()

    mask = torch.zeros((outputPytorch[1].shape[2], outputPytorch[1].shape[2])).cuda().index_put((fx, fy), torch.ones(fx.shape).cuda())
    ...
    # loss_distance = 0.0
    loss = 5.0 * loss_object + labda * (beta * loss_center + loss_z + loss_distance_2 * 0.2)
