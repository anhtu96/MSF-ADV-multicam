...
class attack_msf():
    ...
    def rendering_img(self, xxx):
        ...
        #     i_final, vertex, vertex_orig, face, pytorchModel = self.i_final.data.cuda(), self.object_v.data.cuda(), l_c_c_ori.data.cuda(), self.object_f.data.cuda(), self.LiDAR_model.cuda()
        ...
        #     featureM = gridi2feature_v2(grid_hard, self.direction_val, self.dist_val)
        ...
        #                                                         0.05)
        #     # lossValue = lossValue.data.cpu().numpy()
        #     # loss_object = loss_object.data.cpu().numpy()
        #     # loss_distance = loss_distance.data.cpu().numpy()
        #     # loss_center = loss_center.data.cpu().numpy()
        #     # loss_z = loss_z.data.cpu().numpy()
        #     camera_v = self.object_v.clone()
        #     camera_v = camera_v.permute(1, 0)
...
if __name__ == '__main__':
    ...
    result = differential_evolution(obj.rendering_img, bound, maxiter=10, disp=True, popsize=3)
    # result = differential_evolution(obj.rendering_img, bound)
    # result = differential_evolution(my_obj, bound, workers=1, disp=True, popsize=3, updating='deferred')
