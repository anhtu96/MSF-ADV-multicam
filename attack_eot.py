...
class attack_msf():
    def __init__(self, args):
        ...
        self.eot_samples = self.args.eot_samples
        if self.dataset_name == 'argoverse':
            ...
            self.eot_xrange = self.args.eot_xrange
            self.eot_yrange = self.args.eot_yrange
            self.eot_yawrange = self.args.eot_yawrange

    def add_random_trans(self, x, y, yaw, dist=0):
        dx = 0.5
        dy = 0.5
        dyaw = 5
        # dx = (max(x) - min(x)) / (len(x) - 1)
        # dy = (max(y) - min(y)) / (len(y) - 1)
        # dyaw = (max(yaw) - min(yaw)) / (len(yaw) - 1)
        if dist == 1:
            x += [np.random.uniform(-dx/2, dx/2) for i in range(len(x))]
            y += [np.random.uniform(-dy/2, dy/2) for i in range(len(y))]
            yaw += [np.random.uniform(-dyaw/2, dyaw/2) for i in range(len(yaw))]
        elif dist == 2:
            x += [np.random.normal(0, dx/2) for i in range(len(x))]
            y += [np.random.normal(0, dy/2) for i in range(len(y))]
            yaw += [np.random.normal(0, dyaw/2) for i in range(len(yaw))]
        return x, y, yaw
    ...
    def rendering_img(self):
        ...
        # yaw_angles = np.random.uniform(-self.eot_yawrange, self.eot_yawrange, self.eot_samples)
        # x_shifts = np.random.uniform(-self.eot_xrange, self.eot_xrange, self.eot_samples)
        # y_shifts = np.random.uniform(-self.eot_yrange, self.eot_yrange, self.eot_samples)
        # yaw_angles = np.append(np.linspace(-10, 20, self.eot_samples), np.linspace(40, self.eot_yawrange, self.eot_samples))
        yaw_angles = np.linspace(-10, self.eot_yawrange, self.eot_samples)
        x_shifts = np.linspace(-self.eot_xrange, self.eot_xrange, self.eot_samples)
        y_shifts = np.linspace(-self.eot_yrange, self.eot_yrange, self.eot_samples)
        # x_shifts = [3, 2, 1, 0, 0, 0, 1, 2, 3]
        # y_shifts = [2, 1, 1, 2, 0, 2, -1, -1, -2]
        # yaw_angles = [-30, -20, -15, -5, 0, 5, 15, 20, 30]
        for it in range(iteration):
            ...
            # shape (122999, 4)
            x_shifts_it = np.random.permutation(x_shifts)
            y_shifts_it = np.random.permutation(y_shifts)
            yaw_angles_it = np.random.permutation(yaw_angles)
            # if (it // 50) % 2 == 0:
            #     yaw_angles_it = np.random.permutation(np.linspace(0, 20, self.eot_samples))
            #     # y_shifts_it = np.random.permutation(np.linspace(-self.eot_yrange, 0, self.eot_samples))
            # else:
            #     yaw_angles_it = np.random.permutation(np.linspace(40, self.eot_yawrange, self.eot_samples))
            #     # y_shifts_it = np.random.permutation(np.linspace(0, self.eot_yrange, self.eot_samples))
            # x_shifts_it = x_shifts.copy()
            # y_shifts_it = y_shifts.copy()
            # yaw_angles_it = yaw_angles.copy()
            x_shifts_it, y_shifts_it, yaw_angles_it = self.add_random_trans(x_shifts_it, y_shifts_it, yaw_angles_it, self.args.random_transform)
            # x_shifts_it = np.random.uniform(-2, 2, self.eot_samples)
            # y_shifts_it = np.random.uniform(0, -2, self.eot_samples)
            # yaw_angles_it = np.random.uniform(0, 60, self.eot_samples)
            x_shifts_it = np.append(np.append(x_shifts_it, x_shifts_it), 0)
            y_shifts_it = np.append(np.append(y_shifts_it, -y_shifts_it), 0)
            yaw_angles_it = np.append(np.append(yaw_angles_it, yaw_angles_it), 0)
            # yaw_angles_it = np.append(yaw_angles_it, -yaw_angles_it)
            # x_shifts_it = np.append(x_shifts_it)
            # y_shifts_it = np.append(y_shifts_it)
            # yaw_angles_it = np.append(yaw_angles_it)
            for n_sample in range(len(yaw_angles_it)):
                pts_3d_velo = self.object_v.clone()
                rotation_matrix = R.from_euler('xyz', [0, 0, yaw_angles_it[n_sample]], degrees=True).as_matrix()
                rotation = torch.tensor(rotation_matrix, dtype=torch.float).cuda()
                pts_3d_velo[:, 0] -= self.args.position[0]
                pts_3d_velo[:, 1] -= self.args.position[1]
                pts_3d_velo = torch.matmul(rotation, pts_3d_velo.T).T
                pts_3d_velo[:, 0] += self.args.position[0] + x_shifts_it[n_sample]
                pts_3d_velo[:, 1] += self.args.position[1] + y_shifts_it[n_sample]
                point_cloud = render.render(self.ray_direction, self.length, pts_3d_velo, self.object_f, self.i_final)
                ...
                # camera_v += t
                # self.object_ori = self.object_v.clone()
                # rotation_matrix = R.from_euler('xyz', [0, 0, 45], degrees=True).as_matrix()
                # rotation = torch.tensor(rotation_matrix, dtype=torch.float).cuda()
                # pts_3d_velo[:, 0] -= self.args.position[0]
                # pts_3d_velo[:, 1] -= self.args.position[1]
                # pts_3d_velo = torch.matmul(rotation, pts_3d_velo.T).T
                ...
                final_image = torch.clamp(background_tensor.float(), 0, 1)[None]
                # plt.imsave('final_image.png',final_image[0].detach().cpu().numpy().transpose(1, 2, 0))
            ...
            total_loss /= len(yaw_angles_it)
            if (best_it > total_loss.data.cpu() and self.args.save_best) or \
                    (it == iteration - 1 and not self.args.save_best) or \
                    it == 0:
            ...
            if it % self.args.print_every == 0:
                print('Iter {} of {}: Loss={}'.format(it, iteration, total_loss.data.cpu().numpy()))
            # self.object_v = self.object_v.cuda()
            ...
            # print()
        # torch.cuda.empty_cache()
        print('best loss: {} at iter {}'.format(best_it, best_it_cnt))
        diff = self.object_v - bx
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eot_samples', dest='eot_samples', type=int, default=5)
    parser.add_argument('--eot_xrange', dest='eot_xrange', type=int, default=2)
    parser.add_argument('--eot_yrange', dest='eot_yrange', type=int, default=2)
    parser.add_argument('--eot_yawrange', dest='eot_yawrange', type=int, default=30)
    parser.add_argument('--random_transform', dest='random_transform', type=int, default=0)
